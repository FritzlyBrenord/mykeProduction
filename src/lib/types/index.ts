...
// Types globaux pour Myke Industrie
...
export interface LessonResource {
  ...
  file_name: string;
  file_url: string;
  file_type: 'pdf' | 'zip' | 'docx' | 'xlsx' | 'mp3' | 'other';
  file_size: number;
}
...
// Progrès de leçon
export interface LeconProgress {
  id: string;
  user_id: string;
  lecon_id: string;
  completed: boolean;
...
export const registerSchema = z.object({
  full_name: z.string().min(2, 'Nom minimum 2 caractères'),
  email: z.string().email('Email invalide'),
...
export const commentSchema = z.object({
  content: z.string().min(3, 'Commentaire minimum 3 caractères').max(1000, 'Maximum 1000 caractères'),
...
export const checkoutSchema = z.object({
  ...
  postal_code: z.string().min(3, 'Code postal requis'),
