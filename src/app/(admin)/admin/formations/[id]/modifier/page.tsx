...
interface IntroDraft {
  ...
  intro_video_type: Exclude<VideoType, null>;
}
...
function vm(url: string) {
  return url.match(/vimeo\.com\/(\d+)/)?.[1] ?? null;
}
function isValidYt(url: string) {
  return url.trim().length > 0 && yt(url) !== null;
}
...
const INPUT =
  "w-full px-4 py-2.5 rounded-xl border border-[var(--border)] bg-[var(--background)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] text-[var(--foreground)]";
const LABEL = "block text-sm font-medium mb-1.5 text-[var(--foreground)]";
const CARD =
  "bg-[var(--card)] rounded-2xl p-6 border border-[var(--border)] space-y-4";

const TIMEZONES = [
  ["UTC", "UTC (GMT+0)"],
  ["America/Port-au-Prince", "Haïti (GMT-5/-4) ⭐"],
...
export default function ModifierFormationPage() {
  ...
  const [newLessonByModule, setNewLessonByModule] = useState<
  ...
  const [uploadingModuleIntro, setUploadingModuleIntro] = useState<
    string | null
  >(null);
  const [uploadingModuleIntroProgress, setUploadingModuleIntroProgress] =
    useState(0);
  const [thumbnailFile, setThumbnailFile] = useState<File | null>(null);
  const [thumbnailPreview, setThumbnailPreview] = useState("");
  const [uploadingThumbnail, setUploadingThumbnail] = useState(false);
  const [uploadingThumbnailProgress, setUploadingThumbnailProgress] =
    useState(0);
  ...
  useEffect(() => {
    setLessonDraft(selectedLesson ? { ...selectedLesson } : null);
  }, [selectedLesson]);
    ...
    try {
      ...
      data.modules.forEach((m) => {
        map[m.id] = {
          ...
          intro_video_type: (m.intro_video_type || "youtube") as Exclude<
            VideoType,
            null
          >,
        };
  ...
  const uploadFile = async (
    ...
    if (endpoint === "/api/admin/formations/upload-video") {
      ...
      if (!initResponse.ok || !initPayload?.signedUrl || !initPayload.url) {
        throw new Error(initPayload?.error || "Initialisation upload impossible");
      }
    ...
    return new Promise<{ url: string }>((resolve, reject) => {
      ...
      xhr.onload = () => {
        try {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            const uploadedUrl = payload.url || payload.urls?.[0];
            if (!uploadedUrl) {
              reject(new Error(payload.error || "Upload terminé sans URL"));
              return;
            }
            resolve({ url: uploadedUrl });
          } else {
            reject(new Error(payload.error || "Upload échoué"));
          }
        } catch {
  ...
  const handleSaveFormation = async () => {
    ...
    if (formation.status === "scheduled" && !formation.scheduled_publish_at) {
      toast({
        title: "Erreur",
        description: "Sélectionnez une date de publication.",
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      if (thumbnailFile) {
        ...
        const { url } = await uploadFile(
          thumbnailFile,
          "/api/admin/formations/upload-image",
          setUploadingThumbnailProgress,
        );
        thumbnailUrl = url;
        setThumbnailFile(null);
        setThumbnailPreview("");
        setUploadingThumbnail(false);
        setUploadingThumbnailProgress(0);
      }
  ...
  const publishNow = useCallback(
    async (source: "manual" | "auto" = "manual") => {
      ...
      try {
        const res = await fetch("/api/admin/formations/publish-now", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        });
          ...
          toast({
            ...
            description:
              data.count && data.count > 0
                ? "Formation Publié."
                : "Date non encore atteinte.",
          });
        } else if ((data.published || []).some((p) => p.id === id)) {
          toast({
            title: "✅ Publication automatique",
            description: "La formation est maintenant Publié.",
          });
        }
      } catch (e: any) {
        if (source === "manual")
  ...
  const validateLesson = (): string | null => {
    ...
    if (lessonDraft.video_type === "youtube") {
      if (!lessonDraft.video_url?.trim())
        return "L'URL YouTube est obligatoire";
    ...
    if (lessonDraft.video_type === "upload" && !lessonDraft.video_url?.trim())
      return "Veuillez uploader une vidéo";
    return null;
  };
  ...
  if (!formation)
    return (
      <div className="text-sm text-[var(--muted)]">Formation introuvable.</div>
    );
  ...
  return (
    <div className="space-y-0">
        ...
        <div className="flex items-center gap-3 px-1 pt-2 pb-3">
          ...
          {activeTab === "params" && (
            <Button
              ...
              disabled={savingFormation || uploadingThumbnail}
          ...
          {(
            [
              ...
              },
              {
                ...
                icon: Layers,
                badge:
                  formation.modules.reduce((n, m) => n + m.lecons.length, 0) +
            ...
            >
              ...
              {"badge" in { badge } &&
                typeof badge === "number" &&
                badge > 0 && (
                  <span className="bg-[var(--primary)] text-white text-xs px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                    {badge}
                  </span>
                )}
            </button>
      ...
      {activeTab === "params" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className={CARD}>
              ...
              <div>
                ...
                <p className="text-xs text-[var(--muted)] mt-1">
                  {(formation.description || "").length}/500
                </p>
              </div>
            ...
            <div className={CARD}>
              <h2 className="text-base font-semibold flex items-center gap-2">
                <Upload className="w-4 h-4 text-[var(--primary)]" /> Image de
                couverture
              </h2>
                    ...
                    <div className="w-40">
                      <div className="h-1.5 bg-[var(--background)] rounded-full overflow-hidden border border-[var(--border)]">
                        <div
                          ...
                          style={{ width: `${uploadingThumbnailProgress}%` }}
                        />
                      </div>
                      <p className="text-xs text-[var(--muted)] mt-1">
                        {uploadingThumbnailProgress}%
                      </p>
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className={CARD}>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formation.is_free}
                  onChange={(e) =>
                    setFormation((p) =>
                      p ? { ...p, is_free: e.target.checked } : p,
                    )
                  }
              ...
              {!formation.is_free && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormation((p) =>
                        p ? { ...p, price: parseFloat(e.target.value) } : p,
                      )
                    }
                  />
                </div>
            ...
            <div className={CARD}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormation((p) => (p ? { ...p, format: val } : p))
                      }
                      className={cn(
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormation((p) =>
                      p
                        ? { ...p, duration_hours: parseFloat(e.target.value) }
                        : p,
                    )
                  }
                />
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formation.certificate}
                  onChange={(e) =>
                    setFormation((p) =>
                      p ? { ...p, certificate: e.target.checked } : p,
                    )
                  }
                  className="w-5 h-5 rounded accent-[var(--primary)]"
                />
              ...
              {formation.status === "scheduled" && (
                <div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-xl border-2 border-blue-200 dark:border-blue-800 space-y-3">
                  <h3 className="font-semibold text-blue-900 dark:text-blue-200 text-sm">
                    ðŸ“… Planifier la publication
                  </h3>
                  <div>
                    <label className="block text-xs font-medium mb-1 text-blue-800 dark:text-blue-300">
                      1. Fuseau horaire
                    </label>
                    <select
                      className={cn(INPUT, "text-sm")}
                      value={formation.scheduled_timezone}
                      onChange={(e) =>
                        setFormation((p) =>
                          p ? { ...p, scheduled_timezone: e.target.value } : p,
                        )
                      }
                    >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const iso = e.target.value
                          ...
                          : null;
                        setFormation((p) =>
                          p ? { ...p, scheduled_publish_at: iso } : p,
                        );
                      }}
                    />
                    <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-1">
                      Heure dans le fuseau :{" "}
                      <strong>{formation.scheduled_timezone}</strong>
                    </p>
                  </div>
                  {formation.scheduled_publish_at && (
                    <div className="space-y-2 pt-2 border-t border-blue-200 dark:border-blue-700">
                      ...
                      <Countdown
                        ...
                        timezone={formation.scheduled_timezone}
                        autoTrigger={false}
                      />
                      <button
                        type="button"
                        onClick={() => publishNow("manual")}
      ...
      {/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
          ONGLET CONTENU â€” modules + leçons
      â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */}
      {activeTab === "content" && (
        <div className="flex gap-0 h-[calc(100vh-130px)] pt-4">
          ...
          <div className="w-72 shrink-0 flex flex-col gap-3 pr-4 overflow-y-auto">
            ...
            {formation.modules.length === 0 ? (
              <div className="bg-[var(--card)] rounded-2xl border-2 border-dashed border-[var(--border)] p-8 text-center">
                ...
                <p className="text-xs text-[var(--muted)] mt-1">
                  Créez votre premier module ci-dessus
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 px-3 py-3">
                        ...
                        <span className="text-xs text-[var(--muted)] shrink-0 w-5 text-center">
                          {mi + 1}
                        </span>
                        <input
                          ...
                          onBlur={async (e) => {
                            ...
                            try {
                              await api(
                                `/api/admin/formations/modules/${m.id}`,
                                {
                                  method: "PUT",
                                  headers: {
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            ...
                            className="p-1 text-[var(--muted)] hover:text-[var(--primary)] rounded"
                            title={
                              m.is_visible !== false ? "Masquer" : "Afficher"
                            }
                          >
                            {m.is_visible !== false ? (
                      ...
                      {isOpen && (
                        <div className="border-t border-[var(--border)] px-3 pb-3 pt-2 space-y-3">
                          ...
                          <div className="bg-[var(--background)]/50 rounded-xl border border-[var(--border)] p-2.5 space-y-2">
                                ...
                                {(["none", "text", "video"] as const).map(
                                  (t) => (
                                    <button
                                      ...
                                      className={cn(
                                        "px-1.5 py-0.5 rounded text-[10px] border capitalize transition",
                                        d.intro_type === t
                                          ? "bg-[var(--primary)] text-white border-[var(--primary)]"
                            ...
                            {d.intro_type === "text" && (
                              <textarea
                                className="w-full px-2 py-1.5 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs resize-none"
                                rows={2}
                                value={d.intro_text}
                              ...
                              <div className="space-y-1.5">
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        setIntroDrafts((p) => ({
                                          ...
                                          [m.id]: {
                                            ...d,
                                            intro_video_type: t as any,
                                          },
                                        }))
                                      }
                                      className={cn(
                                        "px-1.5 py-0.5 rounded text-[10px] border capitalize",
                                        d.intro_video_type === t
                                ...
                                {d.intro_video_type === "upload" ? (
                                  <label className="flex items-center gap-1.5 px-2 py-1 bg-[var(--background)] border border-[var(--border)] rounded-lg cursor-pointer text-[10px]">
                                    ...
                                    {uploadingModuleIntro === m.id
                                      ? `Upload ${uploadingModuleIntroProgress}%`
                                      : d.intro_video_url
                                        ? "✅ Uploadée"
                                        : "Choisir vidéo"}
                                    <input
                                      ...
                                      className="hidden"
                                      accept="video/*"
                                      disabled={uploadingModuleIntro === m.id}
                                      onChange={async (e) => {
                                        const f = e.target.files?.[0];
                                        if (!f) return;
                                        try {
                                          setUploadingModuleIntro(m.id);
                                          setUploadingModuleIntroProgress(0);
                                          const { url } = await uploadFile(
                                            f,
                                            "/api/admin/formations/upload-video",
                                            setUploadingModuleIntroProgress,
                                          );
                                          setIntroDrafts((p) => ({
                                            ...p,
                                            [m.id]: {
                                  ...
                                  <input
                                    className="w-full px-2 py-1 rounded-lg border border-[var(--border)] bg-[var(--background)] text-[10px]"
                                    value={d.intro_video_url}
                                    placeholder={
                                      d.intro_video_type === "vimeo"
                                        ? "URL Vimeo"
                                        : "URL YouTube"
                                    }
                                    onChange={(e) =>
                            ...
                            {d.intro_type !== "none" && (
                              <Button
                                size="sm"
                                variant="ghost"
                                className="w-full h-7 text-[10px] font-bold py-0"
                                onClick={async () => {
                                  try {
                                    await api(
                                      `/api/admin/formations/modules/${m.id}`,
                          ...
                          {m.lecons.map((l, li) => (
                            <button
                              ...
                              className={cn(
                                "w-full text-left px-3 py-2 rounded-xl text-xs flex items-center gap-2 transition-colors",
                          ...
                          <div className="flex gap-1.5 pt-1">
                            <input
                              className="flex-1 px-2.5 py-1.5 rounded-lg border border-[var(--border)] bg-[var(--background)] text-xs"
                              value={newLessonByModule[m.id] || ""}
                              placeholder="Titre de la leçon"
                              onChange={(e) =>
                                setNewLessonByModule((p) => ({
                                  ...p,
                                  [m.id]: e.target.value,
                                }))
                              }
                              onKeyDown={async (e) => {
                                ...
                                const title =
                                  (newLessonByModule[m.id] || "").trim() ||
                                  "Nouvelle leçon";
                                try {
                                  const lesson = await api<Lesson>(
                                    `/api/admin/formations/modules/${m.id}/lessons`,
                                    {
                                      method: "POST",
                                      headers: {
                                        "Content-Type": "application/json",
          ...
          {/* â”€â”€ Panneau leçon â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
            ...
            {!selectedLessonId ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-12">
                ...
                <p className="text-sm text-[var(--muted)] mt-2">
                  Choisissez une leçon dans la liste de gauche pour l'éditer
                </p>
              </div>
              ...
              <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 space-y-5">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await api(
                            `/api/admin/formations/lessons/${lessonDraft!.id}`,
                            {
                              method: "PUT",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
                                ...
                                video_type: lessonDraft!.video_type,
                                duration_min: lessonDraft!.duration_min,
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(["upload", "youtube", "vimeo"] as const).map((t) => (
                      <button
                        ...
                        onClick={() =>
                          setLessonDraft((p) =>
                            p ? { ...p, video_type: t, video_url: null } : p,
                          )
                        }
                        className={cn(
                          "px-4 py-2.5 rounded-xl border capitalize text-sm font-medium transition-colors",
                ...
                {lessonDraft?.video_type === "upload" && (
                  <div className="space-y-2">
                    <label className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[var(--border)] cursor-pointer hover:bg-[var(--background)] transition text-sm">
                      ...
                      {uploadingLesson
                        ? `Upload ${uploadingLessonProgress}%...`
                        : lessonDraft.video_url
                          ? "✅ Vidéo uploadée â€” Changer"
                          : "Uploader une vidéo"}
                      <input
                        type="file"
                        className="hidden"
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setUploadingLessonProgress(0);
                            const { url } = await uploadFile(
                              f,
                              "/api/admin/formations/upload-video",
                              setUploadingLessonProgress,
                            );
                            setLessonDraft((p) =>
                              p
                                ? { ...p, video_type: "upload", video_url: url }
                                : p,
                            );
                    ...
                    {uploadingLesson && (
                      <div className="h-1.5 bg-[var(--background)] rounded-full overflow-hidden border border-[var(--border)]">
                        <div
                          className="h-full bg-[var(--primary)] transition-all"
                          style={{ width: `${uploadingLessonProgress}%` }}
                        />
                      </div>
                    )}
                  </div>
                ...
                {(lessonDraft?.video_type === "youtube" ||
                  lessonDraft?.video_type === "vimeo") && (
                  <div>
                    <label className={LABEL}>
                      {lessonDraft.video_type === "youtube"
                        ? "URL YouTube"
                    ...
                    <input
                      className={cn(
                        ...
                        lessonDraft.video_url &&
                          lessonDraft.video_type === "youtube" &&
                          !isValidYt(lessonDraft.video_url)
                          ? "border-red-500"
                          : "",
                        lessonDraft.video_url &&
                          lessonDraft.video_type === "vimeo" &&
                          !vm(lessonDraft.video_url)
                          ? "border-red-500"
                          : "",
                      )}
                      ...
                      onChange={(e) =>
                        setLessonDraft((p) =>
                          p ? { ...p, video_url: e.target.value } : p,
                        )
                      }
                    />
                    {lessonDraft.video_url &&
                ...
                <VideoPreview
                  type={lessonDraft?.video_type || null}
                  url={lessonDraft?.video_url || null}
                />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      className={INPUT}
                      value={lessonDraft?.duration_min ?? 0}
                      onChange={(e) =>
                        setLessonDraft((p) =>
                          p
                  ...
                  <div>
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={lessonDraft?.is_preview || false}
                        onChange={(e) =>
                          setLessonDraft((p) =>
                            p ? { ...p, is_preview: e.target.checked } : p,
                          )
                        }
              ...
              <div className="bg-[var(--card)] rounded-2xl border border-[var(--border)] p-6 space-y-5">
                <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-sm text-[var(--muted)] flex-wrap">
                      ...
                      {selectedLesson?.is_preview && (
                        <span className="text-green-600 dark:text-green-400 flex items-center gap-1">
                          <Eye className="w-3.5 h-3.5" /> Aperçu gratuit
                        </span>
                      )}
                  ...
                  <div className="flex gap-2 shrink-0">
                    <Button
                      size="sm"
                      onClick={() => setEditingLessonId(selectedLessonId)}
                    >
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          await api(
                            `/api/admin/formations/lessons/${selectedLesson?.id}`,
                            {
                              method: "PUT",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
                                is_visible: !(
                                  selectedLesson?.is_visible ?? true
                                ),
                              }),
                            },
                          );
                ...
                {selectedLesson?.video_url && (
                  <VideoPreview
                    type={selectedLesson.video_type}
                ...
                {!selectedLesson?.video_url && !selectedLesson?.content && (
                  <div className="bg-[var(--background)] border-2 border-dashed border-[var(--border)] rounded-xl p-10 text-center">
                    ...
                    <p className="text-sm text-[var(--muted)]">
                      Aucun contenu · Cliquez sur "Éditer" pour commencer
                    </p>
                  </div>
                )}
